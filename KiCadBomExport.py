...
#
#   Required Template Fieldname (within EESchema, Preferences /
#      Schematic Editor Options / Template Field Name)
#   - Mfg_Part_No   Store the manufacturer's part number.  Be specific
#     down to package if you use the online costing, to get specific part
#
...
#	   python "<<path to python script>>KiCadBomExport.py" -i
#	    "%I" <<other cmd line params>>
#       (suggest you don't use the "%O" output file parameter in order
#	to prevent intermediate netlist file being overwritten)
#
#   Troubleshooting:
#   - A log file is created in user home directory
#
#   Further Enhancements Identified:
#   - Add command-line option to cost based on number of boards (currently
#     only assumes 1)
...
#These are the XML Tags used in the KiCad intermediate netlist file.
#  Declared here for ease of maintenance
...
def main(argv):
    ...
    #Which pricing service to use (currently 'F' = FindChips)
    pricingService = ''
    ...
    #    groupParts = True
    apiKey = 'lTnQFAMJIqyRG4rMFGxf'    #Put your API key here
    pricingService= 'F'
	...
	opts, args = getopt.getopt(argv,
	  "hgfa:i:o:", ["help", "group", "apikey=", "input=", "output="])
    ...
    #Perform error checking to make sure cmd-line params passed,
    # files exist etc.
    checkParams(fileIn)
    ...
    if fileOut == '':
	#strip the extension off the Input file
	fileOut = fileIn[:fileIn.rfind('.')] + '_BOM'
    ...
    eTree.parse(fileIn)
    root = eTree.getroot()
    #Find the component-level in the XML Netlist
    section = root.find(tagComponent)

    #Loop through each component
    for component in section:
	#Extract compoonent info and add to listOutput 
	processComponent(listOutput, component, groupParts)

    print("len(listOutput)={0}".format(len(listOutput)))
    print("CSVFieldNames", CSVFieldNames)

    #Get Pricing if we need to.  Need a Manufacturer Part No,
    # and a Pricing Service to be specified
	...
	csvWriter = csv.DictWriter(fOut,
	  delimiter = ',', fieldnames = CSVFieldNames)
    ...
    logger.info('Created CSV file with %i items: %s',
      len(listOutput), fileOut+'.csv')
	...
	#  We do it this way, in order to preserve order of elements -
	# more logical output
	for key in CSVFieldNames:
	    if key in listItem:
		#XML doesn't like spaces in element names,
		# so replace them with "_"
		attribute = SubElement(child,
    ...
    logger.info('Created XML file with %i items: %s',
      len(listOutput), fileOut+'.xml')
...
def processComponent(listOutput, xmlComponent, groupParts):
    ...
    #Component Value
    if xmlComponent.find(tagValue) != None:
	curPart['Value'] = xmlComponent.find(tagValue).text
    if xmlComponent.find(tagFootprint) != None:
	curPart['Footprint'] = xmlComponent.find(tagFootprint).text
    curPart['Count'] = '1'
    ...
    #If user wants parts grouped, then check whether this is a duplicate
    #component. If it is, add this Reference to existing part.
    if groupParts == True:
	...
	for listItem in listOutput:
		...
		if listItem[fldMfgPartNo] == curPart[fldMfgPartNo] and \
		  curPart[fldMfgPartNo] != '-':
		    bDup = True
		    #Add current part reference to the existing part
		    listItem['Reference'] = \
		      listItem['Reference'] + ';' + curPart['Reference']
		    #Add current part value to the existing part
		    listItem['Value'] = \
		      listItem['Value'] + ';' + curPart['Value']
		    #Increase the count of times this part is used
		    listItem['Count'] = str(int(listItem['Count']) + 1)
		    break

	    #No Manufac Part No - so de-dup on Value and Footprint
	    elif listItem['Value'] == curPart['Value'] and \
	      listItem['Footprint'] == curPart['Footprint']:
		bDup = True
		#Add current part reference to the existing part
		listItem['Reference'] = \
		  listItem['Reference'] + ';' + curPart['Reference']
...
def getPricing(listOutput):
	...
	if fldMfgPartNo in listItem:
	    ...
	    params = urllib.urlencode({'apiKey': apiKey,
	      'part': listItem[fldMfgPartNo], 'limit':15,
	      'hostedOnly':'false', 'authorizedOnly':'false',
	      'exactMatch':'true'})
	    #Sent the request
	    ...
	    if r.status != 200:
		logger.error('Error returned by online pricing engine: %s',
		  r.reason)
		logger.info('params: %s', params)
		print("Error returned by pricing service - check log file")
		return
	    else:
		logger.info('Online Pricing: Retrieved for %s',
		  listItem[fldMfgPartNo])
		logger.info('params: %s', params)
		...
		print("message='{0}'".format(metadata["message"]))

	    #Loop through each supplier
	    #
	    #  FUTURE ENHANCEMENT: Compare this to a user-specified
	    # list of suppliers, and only process those
	    #
	    print("len(jdata['response'])", len(jdata['response']))
		...
		for part in supplier['parts']:
		    ...
		    lastPrice = 0
		    lastCurrency = ''
		    #Now we (clumsily) loop through all price-volume breaks,
		    # and find the smallest volume break that meets our
		    # requirements
		    if 'distributorItemNo' in part:
			lastPartNo = part['distributorItemNo']
		    for price in part['price']:
			if price['quantity'] > int(listItem['Count']) and \
			  lastQty > 0:
			    break
		    ...
		    #If we have found a price/volume record, add it to
		    # the current part.
		    if lastQty > 0:
			dist_name = str(supplier['distributor']['name'])
			#Distributor Part No 
			listItem[dist_name+'_PARTNO'] = str(lastPartNo)
			#Quantity
			listItem[dist_name+'_QTY'] = str(lastQty)
			#Currency
			listItem[dist_name+'_CURRENCY'] = str(lastCurrency)
			#Price
			listItem[dist_name+'_PRICE'] = str(lastPrice)
			#Also add this supplier to the Master Field list
			# if it doesn't already exist
			if not(dist_name+'_PARTNO' in CSVFieldNames):
			    CSVFieldNames.append(dist_name+'_PARTNO')
			if not(dist_name+'_QTY' in CSVFieldNames):
			    CSVFieldNames.append(dist_name+'_QTY')
			if not(dist_name+'_PRICE' in CSVFieldNames):
			    CSVFieldNames.append(dist_name+'_PRICE')
			if not(dist_name+'_CURRENCY' in CSVFieldNames):
			    CSVFieldNames.append(dist_name+'_CURRENCY')
...
def printUsage():
    ...
    print("<Output FName>:\tOutput path and filename" +
	  " (no extension) for XML ans CSV")
    print("<API Key>:\tAPI Key for online pricing engine " +
	  "(FindChips / Octopart)")
...
def checkParams(fileIn):
	...
	logger.error('Have asked for online pricing %s but no API Key given.',
	  pricingService) 
	printUsage()
	print('\n***ERROR*** Have asked for online pricing %s' +
	  ' but no API Key given.', pricingService)
	sys.exit(2)
