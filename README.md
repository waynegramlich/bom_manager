...
schematic symbal labels (called references), a PCB footprint,
...
(e.g. resistor, capacitory, transistor, integrated circuit,
...
While all schematics have the graphical reprentations, reference names,
...
(reference name, pin number) pairs that indicate which electical
...
oval/circular/rectangular/trapazoidal copper area which can be
...
copper associated with them and are used for mechanical attachement
...
The advantage of using a logical part number is that manufacuter
...
the BOM managment software can select between different manufacturers
...
Originally, they were specfied by the their length and width in
mils (1 mil = .001" inch.)  Thus, poplular surface mount resistor
...
4. Accesses a part management selection system (currently Octopart)
   to idnentify vendors and prices.
